    ...
    <>
      {userAccount.wallet.length === 0 ? (
        ...
        userAccount.wallet.map((coin) => {
          ...
          return (
            <ul
              className="flex justify-between gap-2 md:gap-5 py-4 px-2 rounded-lg hover:bg-[#23272Eff] transition-all
              
               items-center  md:py-8 text-xs md:text-md relative delay-50 lg:gap-8 md:justify-start"
              key={coin.id}
            >
              <li className="w-1/6">
                <span className="">{coinName} </span>(
                <span className="font-extrabold text-[0.5rem]">
                  {newCoin.symbol}
                </span>
                )
              </li>
