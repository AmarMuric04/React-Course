...
export default function Reviews({ reviews, isScrolled }) {
  ...
  useEffect(() => {
    ...
    setScrollAmount((prevScrollAmount) => {
      ...
      const newScrollAmount = prevScrollAmount + widthOfListItem;
  ...
  return (
    ...
    >
      <ul className="flex gap-4 w-full my-8 items-start">
        {reviews.map((review, index) => {
          return (
            <li
              ...
              ref={listItem}
              className={`min-w-72 hover:bg-[#141210] hover:text-white h-auto border-[0.1rem] border-gray-400 rounded-md use-poppins shadow-xl p-4 
                relative transition-all ${
                  !listItemHovered &&
                  scrollAmount + widthOfListItem * 2 >
