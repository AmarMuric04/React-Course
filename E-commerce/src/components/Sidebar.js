...
export default function Sidebar({ title, iterable }) {
  ...
  const newCategory = iterable ? iterable : categories;

  return (
    <div className="w-[80rem] poppins green-gradient rounded-3xl h-auto py-16 p-8">
      ...
      <div className="w-full flex flex-col items-center mt-8">
        ...
        <ul
          className="m-0 thin-scrollbar p-0 flex pb-4 w-4/5 justify-evenly items-end
         overflow-auto gap-[0.8rem]"
        >
          {newCategory.map((category) => (
            ...
            >
              ...
              >
                <NavLink
                  className={({ isActive }) =>
                    `no-underline transition-all rounded-[2rem] text-black h-full w-full
